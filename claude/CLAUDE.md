...
**コミットまたは PR 作成が完了したタイミングで、必ず `doc-manager` サブエージェントを呼び出してタスクを記録すること。**

### 呼び出しタイミング

| タイミング | 記録内容 |
|---|---|
| `git commit` 成功後 | 実施内容・コミットハッシュ・変更ファイル |
| PR 作成後 | 実施内容・PR URL・変更の意図 |

### 渡す情報

doc-manager に以下を伝えて `task` ドキュメントを作成させる：

- **タイトル**: コミットメッセージまたは PR タイトルをもとにした一行要約
- **実行内容**: 何をしたか（変更ファイル・実装内容）
- **結果**: コミットハッシュまたは PR URL
- **関連**: 対応する issue ID があれば記載

### 呼び出し例

```
doc-manager サブエージェントを使って以下のタスクを記録して：
- タイトル: settings.json のセキュリティフック強化
- 実行内容: Edit/Write/Bash に対するプロジェクト外アクセス制限フックを追加
- 結果: コミット c6ad4f7 / PR #30
```
...
### issue-manager の呼び出しタイミング

**すべてのタスクで以下のライフサイクルを必ず守ること。**

| タイミング | アクション |
|---|---|
| **タスク開始前**（ブランチ作成の前） | `issue-manager` を呼び出して issue を作成する |
| **PR 作成後** | `issue-manager` を呼び出して対応 issue をクローズし、PR URL をコメントで残す |

#### タスク開始前の呼び出し例

```
issue-manager サブエージェントを使って以下の issue を作成して：
- タイトル: settings.json のセキュリティフック強化
- 詳細: Edit/Write/Bash コマンドにプロジェクト外アクセス制限フックを追加する
```

#### PR 作成後の呼び出し例

```
issue-manager サブエージェントを使って issue <ID> をクローズして、
PR URL https://github.com/owner/repo/pull/30 をコメントとして追記して
```
